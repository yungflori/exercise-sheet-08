...
public final class HouseBuilder {
	...
	/**
	 * Check whether the new wall overlaps with any of the already planned walls or
	 * with anything that is already placed on the territory.
	 *
	...
	private boolean wallsOverlap(final Wall newWall) {
		for (Wall wall : walls) {
			if ((this.wallIsBlockedOnTerritory(newWall))
					|| (wall.isVertical() && newWall.isVertical() && this.verticalWallsOverlap(wall, newWall))
					|| (wall.isHorizontal() && newWall.isHorizontal() && this.horizontalWallsOverlap(wall, newWall))
					|| (wall.isHorizontal() && newWall.isVertical() && this.intersectionWallsOverlap(wall, newWall))
					|| (this.intersectionWallsOverlap(newWall, wall))) {
				return true;
			}
		}
		return false;
	...
	/**
	 * Checks whether any of the tiles that the wall would cover is already blocked
	...
	/**
	 * Checks whether a planned vertical wall overlaps with another planned vertical
	 * wall.
	 * 
	...
	private boolean verticalWallsOverlap(final Wall wall, final Wall otherWall) {
		...
		return (wall.getStart().getRow() >= otherWall.getStart().getRow()
				&& wall.getStart().getRow() <= otherWall.getEnd().getRow())
				|| (wall.getEnd().getRow() >= otherWall.getStart().getRow()
						&& wall.getEnd().getRow() <= otherWall.getStart().getRow());
	}

	/**
	 * Checks whether a planned horizontal wall overlaps with another planned
	 * horizontal wall.
	 *
	...
	private boolean horizontalWallsOverlap(final Wall wall, final Wall otherWall) {
		...
		return (wall.getStart().getColumn() >= otherWall.getStart().getColumn()
				&& wall.getStart().getColumn() <= otherWall.getEnd().getColumn())
				|| (wall.getEnd().getColumn() >= otherWall.getStart().getColumn()
						&& wall.getEnd().getColumn() <= otherWall.getStart().getColumn());
	}

	/**
	 * Checks whether a planned horizontal wall overlaps with a planned vertical
	 * wall.
	 * 
